...
[role="_abstract"]
A Helm release is a deployed instance of a Helm chart within your {product-title} cluster. You can install charts in two ways: from the developer catalog or by using a direct chart URL. Using a direct URL eliminates the need for a configured Helm repository by allowing you to pull charts directly from a file service, using HTTP or HTTPS, or from an Open Container Initiative (OCI) registry. Use this method for charts that are not available in your developer catalog or configured repositories.
...
. In the **Chart URL** field, enter the URL for the Helm release you want to install.
+
